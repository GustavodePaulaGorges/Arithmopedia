...
func place_tower(cell_position : Vector2i, tower_type: TowerTypes.TowerType) -> void:
	...
	var new_tower : TowerEntity3 = tower_scene.instantiate()
	add_child(new_tower)
...
func connect_tower_to_spawner(tower: TowerEntity3) -> void:
	var spawner = get_tree().get_first_node_in_group("enemy_spawner")

	if spawner:
		tower.request_spawn_enemy.connect(spawner.spawn_enemy_near_position)

func disconnect_tower_from_spawner(tower: TowerEntity3) -> void:
	var spawner = get_tree().get_first_node_in_group("enemy_spawner")
...
func get_tower_at_position(cell_position : Vector2i) -> TowerEntity3:
