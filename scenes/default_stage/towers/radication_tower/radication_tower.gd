...
func setup_sprite():
	super.setup_sprite()
	sprite.texture = load("res://assets/sprites/TorreRad.png")
...
func select_enemy() -> void:
	...
	var rad_value: int = int(round(sqrt(abs(float(enemy1.value)))))
	if enemy1.value < 0:
		rad_value = -rad_value
	...
	request_spawn_enemy.emit(ratio, rad_value, self, segment_index, branch)
